...
export class ConversationSeedService {
  ...
  async run() {
    ...
    const contactNames = [
      ...
      'William White', 'Linda Harris', 'Richard Clark', 'Patricia Lewis', 'Charles Walker',
      'Barbara Hall', 'Joseph Allen', 'Susan Young', 'Thomas King', 'Jessica Wright',
      'Christopher Scott', 'Nancy Green', 'Daniel Baker', 'Karen Adams', 'Matthew Nelson',
      'Betty Hill', 'Anthony Campbell', 'Helen Mitchell', 'Mark Roberts', 'Dorothy Carter'
    ];
    ...
    for (let i = 0; i < 150; i++) {
      ...
      const lastMessageAt = status === 'closed' 
        ? new Date(createdAt.getTime() + Math.random() * 24 * 60 * 60 * 1000) // within 24 hours
        : new Date(createdAt.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000); // within 7 days
      ...
      let currentTime = new Date(savedConversation.createdAt);
