...
export class KBDocumentsController {
  ...
  async getDocuments(
    ...
    return documents.map(doc => ({
      ...
      metadata: doc.metadata ? {
        ...doc.metadata,
        originalName: doc.metadata.originalName ? decodeFilename(doc.metadata.originalName) : doc.metadata.originalName
      } : doc.metadata
    }));
  }
  ...
  async moveDocument(
    ...
    const userId = req.user.id;
    return this.documentsService.moveToFolder(documentId, userId, folderId || null);
  }
  ...
  async uploadDocument(
    ...
    console.log('🎯 RAW UPLOAD RECEIVED:', {
      ...
      containsNonAscii: [...(file.originalname || '')].some(c => c.charCodeAt(0) > 127),
    ...
    let fileNameFixed = fixEncoding(file.originalname);
    ...
    return {
      ...
      title: createdDoc.title ? decodeFilename(createdDoc.title) : createdDoc.title,
      metadata: createdDoc.metadata ? {
        ...createdDoc.metadata,
        originalName: createdDoc.metadata.originalName ? decodeFilename(createdDoc.metadata.originalName) : createdDoc.metadata.originalName
      } : createdDoc.metadata
    };
  }
