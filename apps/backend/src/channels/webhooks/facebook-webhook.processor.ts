...
export class FacebookWebhookProcessor extends BaseMessageProcessor<FacebookWebhookPayload> {
    ...
    protected extractMessages(payload: FacebookWebhookPayload) {
        ...
        for (const entry of payload.entry || []) {
            ...
            for (const messaging of entry.messaging || []) {
                ...
                if (message?.text) {
                    this.logger.log(`[extractMessages] ✅ Found text message: "${message.text}"`);
                    messages.push({
                        context: {
                ...
                if (messaging.postback) {
                    this.logger.log(`[extractMessages] ✅ Found postback: "${messaging.postback.payload}"`);
                    messages.push({
                        context: {
                            channelId: '',
                            channelType: this.channelType,
        ...
        this.logger.log(`[extractMessages] Total messages extracted: ${messages.length}`);
        return messages;
    }
    ...
    protected async processSingleMessage(
        ...
        if (!channel) {
            ...
            fbChannels.forEach((ch) => {
                this.logger.warn(
                    `  - ${ch.name} (pageId: ${ch.metadata?.pageId})`,
                );
            });
        ...
        this.logger.log(`✅ Channel found: ${channel.name} (ID: ${channel.id})`);
        ...
        // ✅ Broadcast conversation update AFTER message is saved (for conversations list)
        try {
            this.conversationsGateway.broadcastConversationUpdate({
                ...conversation,
                lastMessage: content, // ✅ Include the actual message content
        ...
        this.logger.log(`✅ Events emitted for message ${savedMessage.id}`);
    }
    ...
    protected handleProcessingError(
        ...
        this.logger.error(`Failed to process Facebook webhook for pages: ${pageIds}`);
    }
}
