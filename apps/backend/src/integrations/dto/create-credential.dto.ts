...
export class CreateCredentialDto {
    ...
    @ApiProperty({ example: '1234567890', description: 'Client ID (can use client_id or clientId)' })
    @Transform(({ obj }) => obj.clientId || obj.client_id)
    @IsString()
    clientId: string;

    @ApiProperty({ example: 'secret-key-here', description: 'Client Secret (can use client_secret or clientSecret)' })
