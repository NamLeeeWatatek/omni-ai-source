...
export class WebhookTriggerExecutor implements NodeExecutor {
    async execute(input: NodeExecutionInput): Promise<NodeExecutionOutput> {
        try {
            ...
            const requestPayload = this.buildPayload(payload, input.input, payloadType);

            // Build headers with authentication
            const requestHeaders = this.buildHeaders(headers, authentication, payloadType);
            ...
            const response = await this.executeWithRetry(config, retryCount, retryDelay);
