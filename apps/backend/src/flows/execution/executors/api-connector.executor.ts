...
export class ApiConnectorExecutor implements NodeExecutor {
    async execute(input: NodeExecutionInput): Promise<NodeExecutionOutput> {
        try {
            ...
            const url = this.buildUrl(baseUrl, endpoint, pathParams, queryParams, input.input);
            ...
            if (transformScript) {
                outputData = this.applyTransform(outputData, transformScript, input.input);
            }
    ...
    private buildAuthHeaders(auth: any, inputData: any): Record<string, string> {
        ...
        switch (auth.type) {
            case 'bearer':
                headers['Authorization'] = `Bearer ${this.interpolate(auth.token, inputData)}`;
                break;
            ...
            case 'api-key':
                if (auth.in === 'header') {
                    headers[auth.name || 'X-API-Key'] = this.interpolate(auth.value, inputData);
                }
                break;
            ...
            case 'custom':
                headers[auth.headerName] = this.interpolate(auth.headerValue, inputData);
                break;
        }
            ...
            switch (pagination.type) {
                case 'page':
                    ...
                    if (pagination.limitParam) {
                        url.searchParams.set(pagination.limitParam, String(pagination.limit || 100));
                    }
                    break;
                ...
                case 'offset':
                    ...
                    url.searchParams.set(pagination.limitParam || 'limit', String(pagination.limit || 100));
                    break;

                case 'cursor':
                    if (lastResponse && pagination.cursorPath) {
                        const cursor = this.extractDataByPath(lastResponse.data, pagination.cursorPath);
                        if (cursor) {
     ...
     */
    private checkHasMore(responseData: any, pagination: any, pageData: any): boolean {
