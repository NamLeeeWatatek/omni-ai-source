...
export class ResponseHandlerExecutor implements NodeExecutor {
    async execute(input: NodeExecutionInput): Promise<NodeExecutionOutput> {
        try {
            ...
            result = this.formatOutput(result, outputFormat, outputTemplate, inputData);
    ...
    private applyTransformations(
        ...
        for (const transform of transformations) {
            switch (transform.type) {
                ...
                case 'merge':
                    result = this.mergeWithTemplate(result, transform.template, originalInput);
                    break;
                ...
                case 'calculate':
                    result = this.calculateField(result, transform.expression, transform.outputField);
                    break;

                case 'split':
                    result = this.splitField(result, transform.field, transform.delimiter, transform.outputFields);
                    break;

                case 'join':
                    result = this.joinFields(result, transform.fields, transform.delimiter, transform.outputField);
                    break;
            }
        }
    ...
    private applyAggregation(data: any, aggregate: any): any {
        ...
        const values = aggregate.field
            ? data.map(item => this.getByPath(item, aggregate.field)).filter(v => v != null)
            : data;
     ...
     */
    private evaluateConditions(data: any, conditions: any[]): Record<string, boolean> {
    ...
    private mapObject(item: any, mapping: Record<string, string>, context: any): any {
        const result: Record<string, any> = {};
        for (const [newKey, sourcePath] of Object.entries(mapping)) {
            result[newKey] = this.getByPath(item, sourcePath) ?? this.getByPath(context, sourcePath);
        }
        return result;
    }
    ...
    private sortArray(arr: any[], field: string, order: 'asc' | 'desc' = 'asc'): any[] {
        return [...arr].sort((a, b) => {
    ...
    private calculateField(data: any, expression: string, outputField: string): any {
    ...
    private splitField(data: any, field: string, delimiter: string, outputFields?: string[]): any {
        const value = this.getByPath(data, field);
    ...
    private joinFields(data: any, fields: string[], delimiter: string, outputField: string): any {
        const values = fields.map(f => this.getByPath(data, f)).filter(v => v != null);
    ...
    private applyTemplate(template: any, data: any, context: any): any {
        if (typeof template === 'string') {
            return template.replace(/\{\{([^}]+)\}\}/g, (match, path) => {
                return this.getByPath(data, path.trim()) ?? this.getByPath(context, path.trim()) ?? match;
            });
        }
