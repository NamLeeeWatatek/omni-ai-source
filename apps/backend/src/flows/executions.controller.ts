...
export class ExecutionsController {
  ...
  async findAll(
    ...
    return executions.map((exec) => {
      ...
      const completedNodes = nodeExecutions.filter(n => n.status === 'success').length;
      ...
      try {
        if (exec.endTime && exec.startTime && typeof exec.endTime === 'number' && typeof exec.startTime === 'number') {
      ...
      return {
        ...
        flow_id: exec.flowId,
        status: exec.status,
        startedAt, // Frontend expects this field name
        duration, // Frontend expects this field name
  ...
  async findOne(@Param('id') id: string) {
    ...
    const nodeExecutions = exec.nodeExecutions?.map((node) => {
      ...
      try {
        ...
        console.warn('Invalid startTime for node:', node.nodeId, node.startTime);
      }
      ...
      const execution_time_ms = (node.endTime && node.startTime && typeof node.endTime === 'number' && typeof node.startTime === 'number')
    ...
    try {
      ...
      console.warn('Invalid startTime for execution:', exec.executionId, exec.startTime);
    ...
    try {
      ...
      console.warn('Invalid endTime for execution:', exec.executionId, exec.endTime);
    }
