...
describe('Execution Verification', () => {
    ...
    beforeEach(async () => {
        ...
        strategy.register('webhook', { execute: async (input) => ({ success: true, output: input.input }) });
    });
        ...
        try {
            const flowData = {
                nodes: [
                    ...
                    { id: '2', type: 'code', data: { code: '({ result: input.value * 2 })' } },
                ],
                edges: [
                    { source: '1', target: '2' },
                ],
            ...
            const executionId = await service.executeFlow('test-flow', flowData, inputData);
